...
## 7. Extend Support and Custom Options

Proto2/proto3 `extend` statements for custom options are fully supported:
...
The `ExtendNode` AST node is parsed by `Parser`, and the extension fields are included in the
generated descriptor. Qualified option names of the form `(ext).sub_field` are parsed by
`Parser` into `OptionNode.subFieldPath`, and `DescriptorBuilder+OptionHelpers` converts them
into `Google_Protobuf_UninterpretedOption` with a fully-populated `NamePart` array.

### No Type-Linking (by Design)

SwiftProtoParser does **not** perform type-linking — the post-parse step in which a compiler
resolves custom option extension names to their concrete field types and re-encodes the option
values as typed extension fields.

Consequences of this design decision:

| Aspect | SwiftProtoParser | protoc |
|---|---|---|
| Standard options (`deprecated`, `go_package`, …) | Typed fields | Typed fields |
| Custom extension options (`(my_ext).field`) | `uninterpreted_option` with NameParts | Typed extension fields |
| Import resolution required for options? | No | Yes |

This is the same approach taken by [`bufbuild/protocompile`](https://github.com/bufbuild/protocompile)
in unlinked mode, and covers all known use cases for schema analysis, documentation generation,
and code generation tools.

If your use case requires fully resolved option types, use `protoc --descriptor_set_out` to
produce a `FileDescriptorSet` and load that directly.
...
Each layer is tested in isolation with mocks/stubs where needed. Every `ProtoParseError` case
has an explicit test. Target: ≥ 92% region coverage, 95% line coverage.

### Integration Tests — Comparison Approach

All integration tests perform **byte-for-byte comparison** of serialised
`Google_Protobuf_FileDescriptorProto` values. JSON diffs are emitted as diagnostic output when
bytes differ, but the canonical assertion is always on binary equality.

Before comparing, both descriptors are normalised:
- `source_code_info` is cleared (only present with `--include_source_info`).
- `edition` is cleared (editions-only field, not yet supported).

### Tier 1a — Standard Options (protoc reference)

Files containing only standard built-in options are compared byte-for-byte against descriptors
generated by `protoc --include_imports --descriptor_set_out`.

Reference files live in:
- `Tests/TestResources/WellKnownDescriptors/` — all `google/protobuf/*.proto` types.
- `Tests/TestResources/HandcraftedDescriptors/` — hand-crafted protos with standard options.
- `Tests/TestResources/ClientProtoDescriptors/` — real-world client protos without custom options.

### Tier 1b — Custom Extension Options (proto-unlinked-gen reference)

Files containing `(ext).field`-style custom options are compared against descriptors produced
by `Scripts/proto-unlinked-gen/` — a helper CLI built on `bufbuild/protocompile` that runs the
parser without the linking stage. Both tools store custom options as `uninterpreted_option` with
populated `NamePart` arrays.

Because the Go-tool stores ALL options (including standard ones like `go_package`) as
`uninterpreted_option`, while SwiftProtoParser correctly places them in typed fields, the Tier 1b
comparison filters to **extension options only** (entries where `name[0].isExtension == true`).

Reference files live in:
- `Tests/TestResources/HandcraftedDescriptors/` — hand-crafted protos with custom options.
- `Tests/TestResources/ClientProtoDescriptors/` — client protos with `buf.validate` and `google.api.http` custom options.

### Reference generation scripts

```bash
Scripts/generate_well_known_descriptors.sh      # Tier 1a: well-known types
Scripts/generate_extension_option_descriptors.sh  # Tier 1a + 1b: all other reference files
```
