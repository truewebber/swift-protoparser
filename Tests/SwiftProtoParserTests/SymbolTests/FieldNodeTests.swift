...
final class FieldNodeTests: XCTestCase {
    ...
    func testFieldWithPackedOption() throws {
        ...
        // Validation should not throw
        XCTAssertNoThrow(try fieldNode.validate(), "Valid field with packed option should pass validation")
    }
            ...
            XCTAssertThrowsError(try invalidMapField.validate()) { error in
                ...
                if let parserError = error as? ParserError {
                    ...
                    case .invalidMapKeyType(let type):
                        XCTAssertTrue(type.contains(String(describing: invalidKeyType)), "Error should mention the invalid key type")
                    default:
        ...
        XCTAssertThrowsError(try repeatedMapField.validate()) { error in
            ...
            if let parserError = error as? ParserError {
                ...
                case .custom(let message):
                    ...
                    XCTAssertTrue(message.contains("cannot be repeated"), "Error should mention that maps cannot be repeated")
                default:
