...
final class SymbolResolutionTests: XCTestCase {
    ...
    /**
     * Test adding and resolving extension nodes
     *
     * This test verifies that the symbol table can add and resolve extension nodes.
     */
    func testAddAndResolveExtendNode() throws {
        ...
        try symbolTable.addExtension(field1, extendedType: "example.Person", package: "example.extensions")
        try symbolTable.addExtension(field2, extendedType: "example.Person", package: "example.extensions")
        ...
        XCTAssertTrue(symbolTable.isExtension("example.extensions.extra_field"), "extra_field should be recognized as an extension")
        XCTAssertTrue(symbolTable.isExtension("example.extensions.another_field"), "another_field should be recognized as an extension")
        
        // Verify hasExtension method
        XCTAssertTrue(symbolTable.hasExtension(for: "example.Person", named: "extra_field"), "Should find extension by name")
        XCTAssertTrue(symbolTable.hasExtension(for: "example.Person", named: "another_field"), "Should find extension by name")
        XCTAssertFalse(symbolTable.hasExtension(for: "example.Person", named: "nonexistent"), "Should not find nonexistent extension")
    ...
    /**
     * Test field resolution in message types
     *
     * This test verifies that the symbol table can resolve fields in message types.
     */
    func testFieldResolution() throws {
        ...
        try symbolTable.addSymbol(nameField, kind: .field, package: "example", parent: symbolTable.lookup("example.Person"))
        try symbolTable.addSymbol(ageField, kind: .field, package: "example", parent: symbolTable.lookup("example.Person"))
        
        // Verify hasField method
        XCTAssertTrue(symbolTable.hasField(in: "example.Person", named: "name"), "Should find field by name")
        XCTAssertTrue(symbolTable.hasField(in: "example.Person", named: "age"), "Should find field by name")
        XCTAssertFalse(symbolTable.hasField(in: "example.Person", named: "nonexistent"), "Should not find nonexistent field")
     ...
     *
     * This test verifies that the symbol table can retrieve symbols by kind.
     */
    ...
    func testClearSymbolTable() throws {
        ...
        XCTAssertEqual(symbolTable.getSymbols(ofKind: .message).count, 0, "Should have no messages after clearing")
        XCTAssertEqual(symbolTable.lookupExtensions(for: "example.Person").count, 0, "Should have no extensions after clearing")
    }
     ...
     *
     * This test verifies that the symbol table throws an error when adding a duplicate symbol.
     */
     ...
     *
     * This test verifies that the symbol table throws an error when adding a duplicate extension.
     */
    func testDuplicateExtensionError() throws {
        ...
        try symbolTable.addExtension(field, extendedType: "example.Person", package: "example.extensions")
        
        // Try to add the same extension again
        XCTAssertThrowsError(try symbolTable.addExtension(field, extendedType: "example.Person", package: "example.extensions")) { error in
            guard case SymbolTableError.duplicateSymbol(let name) = error else {
                XCTFail("Expected duplicateSymbol error, got \(error)")
                return
            }
            
            XCTAssertEqual(name, "example.extensions.extra_field", "Error should contain the duplicate extension name")
        }
    }
} 