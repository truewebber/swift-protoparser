...
final class KeywordRecognizerTests: XCTestCase {
  ...
  func testCaseSensitivity() {
    ...
    for (variant, shouldBeKeyword) in variations {
      ...
      if shouldBeKeyword {
        XCTAssertTrue(
          token.isLiteral == false && token.isIgnorable == false,
  ...
  func testIsValidIdentifier() {
    ...
    for keyword in ProtoKeyword.allCases {
      XCTAssertFalse(
        KeywordRecognizer.isValidIdentifier(keyword.rawValue),
    ...
    for identifier in validIdentifiers {
      XCTAssertTrue(
        KeywordRecognizer.isValidIdentifier(identifier),
  ...
  func testValidateIdentifierName() {
    ...
    for keyword in ProtoKeyword.allCases {
      XCTAssertFalse(
        KeywordRecognizer.validateIdentifierName(keyword.rawValue),
    ...
    for name in validNames {
      XCTAssertTrue(
        KeywordRecognizer.validateIdentifierName(name),
  ...
  func testCoreKeywords() {
    ...
    for keyword in coreKeywords {
      XCTAssertTrue(
        KeywordRecognizer.isCoreKeyword(keyword),
  ...
  func testTypeDefinitionKeywords() {
    ...
    for keyword in typeKeywords {
      XCTAssertTrue(
        KeywordRecognizer.isTypeDefinitionKeyword(keyword),
  ...
  func testFieldModifierKeywords() {
    ...
    for keyword in modifierKeywords {
      XCTAssertTrue(
        KeywordRecognizer.isFieldModifierKeyword(keyword),
  ...
  func testServiceKeywords() {
    ...
    for keyword in serviceKeywords {
      XCTAssertTrue(
        KeywordRecognizer.isServiceKeyword(keyword),
  ...
  func testProto2CompatibilityKeywords() {
    ...
    for keyword in proto2Keywords {
      XCTAssertTrue(
        KeywordRecognizer.isProto2CompatibilityKeyword(keyword),
  ...
  func testAllKeywordStrings() {
    ...
    XCTAssertEqual(
      keywordStrings.count,
      ProtoKeyword.allCases.count,
  ...
  func testKeywordCheckPerformance() {
    let testStrings = ProtoKeyword.allCases.map { $0.rawValue } + ["MyMessage", "field_name", "user_id", "not_keyword"]
