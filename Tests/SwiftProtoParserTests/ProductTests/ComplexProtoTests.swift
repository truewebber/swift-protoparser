...
final class ComplexProtoTests: XCTestCase {
  ...
  func testEdgeCasesParsing() throws {
    ...
    case .success(let ast):
      ...
      for message in ast.messages {
        for field in message.fields where field.number > 1000 {
  ...
  func testProductionGRPCServiceParsing() throws {
    ...
    case .success(let ast):
      ...
      print(
        "✅ CRITICAL SUCCESS: Production gRPC Service parsing - ALL streaming types, qualified types, oneof, and maps verified!"
      )
  ...
  func testMalformedProtoErrorHandling() throws {
    ...
    case .failure(let error):
      ...
      XCTAssertTrue(
        error.localizedDescription.contains("error") || error.localizedDescription.contains("syntax")
