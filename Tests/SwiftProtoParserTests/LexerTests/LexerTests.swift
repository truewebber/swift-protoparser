...
final class LexerTests: XCTestCase {
  ...
  func testSingleLineComments() throws {
    ...
    for input in inputs {
      ...
      XCTAssertEqual(tokens[0].type, .eof)
      XCTAssertEqual(
        tokens[0].leadingComments.first, input.dropFirst(2).trimmingCharacters(in: .whitespaces))
    }
  }
    ...
    for input in invalidInputs {
      XCTAssertThrowsError(try getAllTokens(from: input)) { error in
        ...
        case .invalidCharacter(let char, let location):
          ...
          XCTAssertEqual(location.line, 1)
          let expectedColumn =
            input.distance(from: input.startIndex, to: input.firstIndex(of: invalidChar)!) + 1
  ...
  func testSpecialWhitespaceCharacters() throws {
    let input =
      "message\u{00A0}Example\u{2002}{\u{2003}string\u{2004}name\u{2005}=\u{2006}1;\u{2007}}"
