...
final class LexerTests: XCTestCase {
  ...
  ///
  /// This test verifies that the lexer correctly tokenizes keywords.
  func testKeywords() throws {
    ...
    for (index, token) in tokens.enumerated() {
      XCTAssertEqual(
        token.type,
        expectedTokenTypes[index],
        "Token at index \(index) should be \(expectedTokenTypes[index]), but got \(token.type)"
      )
    }
  }
  ...
  func testIdentifiers() throws {
    ...
    for (index, identifier) in identifiers.enumerated() {
      XCTAssertEqual(
        ...
        "Identifier at index \(index) should be \(expectedIdentifiers[index]), but got \(identifier)"
      )
    }
  }
  ...
  func testStringLiterals() throws {
    ...
    for (index, string) in strings.enumerated() {
      XCTAssertEqual(
        ...
        "String at index \(index) should be \(expectedStrings[index]), but got \(string)"
      )
    }
  }
  ...
  ///
  /// This test verifies that the lexer correctly tokenizes integer literals.
  func testNumberLiterals() throws {
    ...
    for (index, number) in numbers.enumerated() {
      XCTAssertEqual(
        ...
        "Number at index \(index) should be \(expectedNumbers[index]), but got \(number)"
      )
    }
  }
  ...
  ///
  /// This test verifies that the lexer correctly tokenizes operators.
  ...
  ///
  /// This test verifies that the lexer correctly tokenizes comments.
  ...
  func testLineAndColumnTracking() throws {
    ...
    for (index, token) in tokens.enumerated() {
      ...
      XCTAssertEqual(
        ...
        "Token at index \(index) should be at line \(expectedPosition.line), but got \(token.location.line)"
      )
      XCTAssertEqual(
        token.location.column,
        expectedPosition.column,
        "Token at index \(index) should be at column \(expectedPosition.column), but got \(token.location.column)"
      )
    }
  }
  ...
  /// Test tokenizing invalid tokens.
  ///
  /// This test verifies that the lexer correctly handles invalid tokens.
  ...
  /// Test tokenizing an empty file.
  ///
  /// This test verifies that the lexer correctly tokenizes an empty file.
  ...
  /// Test tokenizing a file with only whitespace.
  ///
  /// This test verifies that the lexer correctly tokenizes a file with only whitespace.
  func testWhitespaceAndCommentsOnly() throws {
    ...
    XCTAssertEqual(
      token.type,
      .eof,
      "Expected EOF token for input with only whitespace and comments"
    )
  }

  /// Test tokenizing extremely long identifiers.
  ///
  ...
  /// Test tokenizing adjacent punctuation.
  ///
