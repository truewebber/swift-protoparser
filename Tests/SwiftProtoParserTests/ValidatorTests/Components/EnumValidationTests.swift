...
final class EnumValidationTests: XCTestCase {
    ...
    func testInvalidEnumNames() throws {
        ...
        for enumName in invalidEnumNames {
            ...
            XCTAssertThrowsError(try validator.validate(file), "Enum name '\(enumName)' should be invalid") { error in
                guard let validationError = error as? ValidationError else {
    ...
    func testValidEnumValueNames() throws {
        ...
        for valueName in validEnumValueNames {
            ...
            // This should not throw
            XCTAssertNoThrow(try validator.validate(file), "Enum value name '\(valueName)' should be valid")
        }
    }
    ...
    func testInvalidEnumValueNames() throws {
        ...
        for valueName in invalidEnumValueNames {
            ...
            XCTAssertThrowsError(try validator.validate(file), "Enum value name '\(valueName)' should be invalid") { error in
                guard let validationError = error as? ValidationError else {
