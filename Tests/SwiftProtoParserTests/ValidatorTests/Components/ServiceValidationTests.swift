...
final class ServiceValidationTests: XCTestCase {
    ...
    func testMethodNameValidation() throws {
        ...
        for name in validMethodNames {
            ...
            XCTAssertNoThrow(try serviceValidator.validateMethodUniqueness(service), "Method name '\(name)' should be valid")
        }
        ...
        for name in invalidMethodNames {
            ...
            XCTAssertThrowsError(try serviceValidator.validateMethodUniqueness(service), "Method name '\(name)' should be invalid") { error in
    ...
    func testStreamingMethodValidation() throws {
        ...
        for config in streamingConfigs {
            ...
            // This should not throw
            XCTAssertNoThrow(try serviceValidator.validateStreamingRules(rpc), "\(config.description) method should be valid")
        }
    }
    ...
    func testRPCTypeValidation() throws {
        ...
        XCTAssertThrowsError(try state.registerType("test.TestRequest", node: requestMessage), "Registering the same type twice should throw an error")
        ...
        XCTAssertNoThrow(try state.registerType("test.TestResponse", node: responseMessage), "Registering a different type should not throw")
    }
    ...
    func testRPCOptionsValidation() throws {
        ...
        XCTAssertEqual(serviceWithValidRPCOptions.rpcs[0].options[0].name, "deprecated", "First option should be 'deprecated'")
        XCTAssertEqual(serviceWithValidRPCOptions.rpcs[0].options[1].name, "idempotency_level", "Second option should be 'idempotency_level'")
        ...
        XCTAssertEqual(serviceWithInvalidRPCOptions.rpcs[0].options.count, 1, "RPC should have 1 option")
        XCTAssertEqual(serviceWithInvalidRPCOptions.rpcs[0].options[0].name, "invalid_option", "Option should be 'invalid_option'")
    }
    ...
    func testServiceOptionsValidation() throws {
        ...
        XCTAssertEqual(serviceWithValidOptions.options[0].name, "deprecated", "Option should be 'deprecated'")
        ...
        XCTAssertEqual(serviceWithInvalidOptions.options[0].name, "invalid_option", "Option should be 'invalid_option'")
    }
} 