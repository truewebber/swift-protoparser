...
final class MessageValidationTests: XCTestCase {
    ...
    func testValidMessageNames() throws {
        ...
        for messageName in validMessageNames {
            ...
            XCTAssertNoThrow(try validator.validate(file), "Message name '\(messageName)' should be valid")
        }
    }
    ...
    func testInvalidMessageNames() throws {
        ...
        for messageName in invalidMessageNames {
            ...
            XCTAssertThrowsError(try validator.validate(file), "Message name '\(messageName)' should be invalid") { error in
    ...
    func testFieldNumberRange() throws {
        ...
        let validFieldNumbers = [1, 2, 10, 100, 1000, 10000, 100000, 536870911]
        ...
        let invalidFieldNumbers = [0, -1, -10, 536870912, 1000000000]
        ...
        for fieldNumber in invalidFieldNumbers {
            ...
            XCTAssertThrowsError(try validator.validate(file), "Field number \(fieldNumber) should be invalid") { error in
                guard let validationError = error as? ValidationError else {
        ...
        for fieldNumber in reservedFieldNumbers {
            ...
            // This should throw an invalidFieldNumber error
            XCTAssertThrowsError(try validator.validate(file), "Field number \(fieldNumber) should be invalid (reserved)") { error in
                guard let validationError = error as? ValidationError else {
                    XCTFail("Expected ValidationError for field number \(fieldNumber)")
                    return
                }
    ...
    func testInvalidFieldNames() throws {
        ...
        for fieldName in invalidFieldNames {
            ...
            XCTAssertThrowsError(try validator.validate(file), "Field name '\(fieldName)' should be invalid") { error in
                guard let validationError = error as? ValidationError else {
