...
final class OptionValidationTests: XCTestCase {
    ...
    func testInvalidFileOptions() throws {
        ...
        for option in invalidOptions {
            ...
            XCTAssertThrowsError(try validator.validate(file), "Option '\(option.name)' with value '\(option.value)' should be invalid") { error in
                ...
                case .invalidOptionValue(let message):
                    XCTAssertTrue(message.contains(option.errorType), "Error message should mention '\(option.errorType)'")
                case .unknownOption(let name):
                    XCTAssertEqual(name, option.name)
                default:
                    XCTFail("Expected invalidOptionValue or unknownOption error for '\(option.name)', got \(validationError)")
                }
            }
        }
    }
    ...
    func testValidMessageOptions() throws {
        ...
        for option in validOptions {
            ...
            // This should not throw
            XCTAssertNoThrow(try validator.validate(file), "Message option '\(option.name)' should be valid")
        }
    }
    ...
    func testValidFieldOptions() throws {
        ...
        for option in validOptions {
            ...
            // This should not throw
            XCTAssertNoThrow(try validator.validate(file), "Field option '\(option.name)' should be valid")
        }
    }
    ...
    func testInvalidEnumOptions() throws {
        ...
        for option in invalidOptions {
            ...
            // This should throw an error
            XCTAssertThrowsError(try validator.validate(file), "Enum option '\(option.name)' with value '\(option.value)' should be invalid") { error in
                guard let validationError = error as? ValidationError else {
    ...
    func testCustomOptions() throws {
        ...
        XCTAssertNoThrow(try validator.validate(fileWithCustomOption), "Custom options should be allowed without validation")
    }
} 