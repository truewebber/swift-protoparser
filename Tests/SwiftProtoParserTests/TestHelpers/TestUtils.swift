...
enum TestUtils {
    ...
    static func createTemporaryFile(with content: String, fileExtension: String = "proto") throws -> URL {
        let temporaryDirectory = FileManager.default.temporaryDirectory
        let fileName = UUID().uuidString
        let fileURL = temporaryDirectory.appendingPathComponent(fileName).appendingPathExtension(fileExtension)
    ...
    static func assertThrows<T: Error, R>(
        ...
        XCTAssertThrowsError(try expression(), message(), file: file, line: line) { error in
            XCTAssertTrue(error is T, "Expected error of type \(T.self), but got \(type(of: error))", file: file, line: line)
        }
    }
    ...
    static func assertThrows<T: Error & Equatable, R>(
        ...
        XCTAssertThrowsError(try expression(), message(), file: file, line: line) { error in
            guard let typedError = error as? T else {
                XCTFail("Expected error of type \(T.self), but got \(type(of: error))", file: file, line: line)
                return
            }
            
            XCTAssertEqual(typedError, expectedError, "Expected error \(expectedError), but got \(typedError)", file: file, line: line)
        }
    }
}
