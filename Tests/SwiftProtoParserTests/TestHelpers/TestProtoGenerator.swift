...
struct TestProtoGenerator {
  ...
  private static func generateMethod(index: Int, includeOptions: Bool) -> String {
    ...
    var method =
      "rpc Method\(index)(\(inputStreaming)Message0) returns (\(outputStreaming)Message0);"

    // Add options if needed
    if includeOptions {
      method =
        "rpc Method\(index)(\(inputStreaming)Message0) returns (\(outputStreaming)Message0) { option deprecated = true; };"
    }
