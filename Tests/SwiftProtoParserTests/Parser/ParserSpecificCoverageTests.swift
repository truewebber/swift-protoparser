...
final class ParserSpecificCoverageTests: XCTestCase {
  ...
  func testMapTypeParsing() {
    let mapTypeCases = [
      ...
      (
        "enum_value",
        "syntax = \"proto3\"; enum Status { UNKNOWN = 0; } message T { map<string, Status> statuses = 1; }"
      ),
    ]
  ...
  func testReservedFieldNames() {
    let reservedNameCases = [
      ...
      (
        "separate_reserved",
        "syntax = \"proto3\"; message T { reserved 1 to 5; reserved \"old_field\"; string name = 6; }"
      ),
    ]
