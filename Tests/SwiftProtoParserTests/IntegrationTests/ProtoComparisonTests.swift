...
final class ProtoComparisonTests: XCTestCase {
  ...
  private func compareProtoOutput(protoFile: String) throws {
    ...
    XCTAssertEqual(
      protocDescriptorSet.file.count,
      1,
      "Expected exactly one file descriptor in protoc output"
    )

    // Compare the descriptors
    compareDescriptors(
      swiftDescriptor: descriptors,
      protocDescriptor: protocDescriptorSet.file[0],
      protoFile: protoFile
    )
  }
  ...
  private func compareDescriptors(
    ...
    protocDescriptor: Google_Protobuf_FileDescriptorProto,
    protoFile: String
  ) {
    ...
    XCTAssertEqual(
      swiftDescriptor.package,
      protocDescriptor.package,
      "Package name mismatch for \(protoFile)"
    )
    ...
    for (index, enumType) in swiftDescriptor.enumType.enumerated() where index < protocDescriptor.enumType.count {
      compareEnumTypes(
        swiftEnum: enumType,
        protocEnum: protocDescriptor.enumType[index],
        protoFile: protoFile
      )
    }
    ...
    for i in 0..<minMessageCount {
      compareMessageTypes(
        swiftMessage: swiftMessages[i],
        protocMessage: protocMessages[i],
        protoFile: protoFile,
        parentName: ""
      )
    }
    ...
    XCTAssertEqual(
      swiftDescriptor.service.count,
      protocDescriptor.service.count,
    ...
    for (index, service) in swiftDescriptor.service.enumerated() {
      compareServiceTypes(
        ...
        protocService: protocDescriptor.service[index],
        protoFile: protoFile
      )
    }
  }
  ...
  private func compareMessageTypes(
    swiftMessage: Google_Protobuf_DescriptorProto,
    protocMessage: Google_Protobuf_DescriptorProto,
    protoFile: String,
    parentName: String
  ) {
    ...
    for i in 0..<minFieldCount {
      compareFieldTypes(
        swiftField: swiftFields[i],
        protocField: protocFields[i],
        protoFile: protoFile,
    ...
    for i in 0..<minNestedCount {
      compareMessageTypes(
        swiftMessage: swiftNestedMessages[i],
        protocMessage: protocNestedMessages[i],
        protoFile: protoFile,
        parentName: fullName
      )
    }
    ...
    for (index, enumType) in swiftMessage.enumType.enumerated() where index < protocMessage.enumType.count {
      compareEnumTypes(
        swiftEnum: enumType,
        protocEnum: protocMessage.enumType[index],
        protoFile: protoFile
      )
    }
  }
  ...
  private func compareFieldTypes(
    ...
    protocField: Google_Protobuf_FieldDescriptorProto,
    protoFile: String,
    messageName: String
  ) {
  ...
  private func compareEnumTypes(
    swiftEnum: Google_Protobuf_EnumDescriptorProto,
    protocEnum: Google_Protobuf_EnumDescriptorProto,
    protoFile: String
  ) {
    ...
    // Compare enum values
    XCTAssertEqual(
      swiftEnum.value.count,
      protocEnum.value.count,
      "Enum value count mismatch for \(protoFile)"
    )
    for i in 0..<min(swiftEnum.value.count, protocEnum.value.count) {
      XCTAssertEqual(
        swiftEnum.value[i].name,
        protocEnum.value[i].name,
        "Enum value name mismatch for \(protoFile).\(swiftEnum.value[i].name)"
      )
      XCTAssertEqual(
        swiftEnum.value[i].number,
        protocEnum.value[i].number,
  ...
  private func compareServiceTypes(
    ...
    protocService: Google_Protobuf_ServiceDescriptorProto,
    protoFile: String
  ) {
    ...
    // Compare methods
    XCTAssertEqual(
      swiftService.method.count,
      protocService.method.count,
      "Method count mismatch for \(protoFile)"
    )
    for i in 0..<min(swiftService.method.count, protocService.method.count) {
      XCTAssertEqual(
        swiftService.method[i].name,
        protocService.method[i].name,
        "Method name mismatch for \(protoFile).\(swiftService.method[i].name)"
      )
      XCTAssertEqual(
        swiftService.method[i].inputType,
        protocService.method[i].inputType,
        "Method input type mismatch for \(protoFile).\(swiftService.method[i].name)"
      )
      XCTAssertEqual(
        swiftService.method[i].outputType,
        protocService.method[i].outputType,
        "Method output type mismatch for \(protoFile).\(swiftService.method[i].name)"
      )
      XCTAssertEqual(
        swiftService.method[i].clientStreaming,
        protocService.method[i].clientStreaming,
        "Method client streaming mismatch for \(protoFile).\(swiftService.method[i].name)"
      )
      XCTAssertEqual(
        swiftService.method[i].serverStreaming,
        protocService.method[i].serverStreaming,
