...
final class ConfigurationTests: XCTestCase {
  ...
  func testDefaultConfiguration() {
    ...
    XCTAssertFalse(
      ...
      "Unknown fields should not be preserved by default"
    )
  }
  ...
  func testDefaultNamingOptions() {
    ...
    // Assert
    XCTAssertTrue(
      config.namingOptions.enforceConventions,
      "Naming conventions should be enforced by default"
    )
    XCTAssertTrue(
      config.namingOptions.allowUnderscoresInTypes,
      "Underscores in types should be allowed by default"
    )
    XCTAssertTrue(
      config.namingOptions.allowUnderscoresInFields,
      "Underscores in fields should be allowed by default"
    )
  }
  ...
  func testBuilderWithNamingOptions() {
    ...
    XCTAssertFalse(
      config.namingOptions.enforceConventions,
      "Naming conventions should not be enforced"
    )
    XCTAssertFalse(
      config.namingOptions.allowUnderscoresInTypes,
      "Underscores in types should not be allowed"
    )
    XCTAssertFalse(
      config.namingOptions.allowUnderscoresInFields,
      "Underscores in fields should not be allowed"
    )
  }
