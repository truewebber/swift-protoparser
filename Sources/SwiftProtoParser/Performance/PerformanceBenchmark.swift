...
public final class PerformanceBenchmark {
  ...
  public struct BenchmarkResult {
    ...
    public var medianDuration: TimeInterval {
      ...
      return count % 2 == 0 ? 
        (successfulDurations[count/2 - 1] + successfulDurations[count/2]) / 2 :
        successfulDurations[count/2]
    }
    ...
    public var standardDeviation: TimeInterval {
      ...
      let variance = successfulDurations.reduce(0) { sum, duration in
    ...
    public var isAcceptable: Bool {
      return successRate >= 0.95 && // 95% success rate
             averageDuration <= configuration.maxParsingTime &&
             averageMemoryUsage <= configuration.maxMemoryUsage
    }
  }
    ...
    for currentResult in current.results {
      if let baselineResult = baseline.results.first(where: { $0.operation == currentResult.operation }) {
        ...
        if performanceRatio > 1.1 { // More than 10% slower
          regressions.append("\(currentResult.operation): \(String(format: "%.1f", (performanceRatio - 1) * 100))% slower")
        } else if performanceRatio < 0.9 { // More than 10% faster
          improvements.append("\(currentResult.operation): \(String(format: "%.1f", (1 - performanceRatio) * 100))% faster")
        }
      }
    }
  ...
  private func measureOperation<T>(_ operation: () -> Result<T, ProtoParseError>) -> (duration: TimeInterval, memoryUsage: Int64, success: Bool, error: ProtoParseError?) {
    let startTime = Date()
  ...
  private func findProtoFiles(in directory: String) throws -> [String] {
    ...
    guard let enumerator = fileManager.enumerator(
      ...
      throw ProtoParseError.ioError(underlying: NSError(domain: "PerformanceBenchmark", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to enumerate directory"]))
    }
