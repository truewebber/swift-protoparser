...
public final class SourceInfoGenerator {
    ...
    private func processEnumNode(_ enum_: EnumNode) {
        addLocation(for: enum_)

        // Process values
        for (index, value) in enum_.values.enumerated() {
        ...
        // Process options
        for (index, option) in enum_.options.enumerated() {
