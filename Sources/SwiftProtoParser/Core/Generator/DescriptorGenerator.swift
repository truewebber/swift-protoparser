...
public final class DescriptorGenerator {
    ...
    private func generateMessageDescriptor(_ message: MessageNode) throws -> Google_Protobuf_DescriptorProto {
    ...
    private func generateFieldDescriptor(_ field: FieldNode) throws -> Google_Protobuf_FieldDescriptorProto {
    ...
    private func generateEnumDescriptor(_ enumType: EnumNode) throws -> Google_Protobuf_EnumDescriptorProto {
    ...
    private func generateServiceDescriptor(_ service: ServiceNode) throws -> Google_Protobuf_ServiceDescriptorProto {
    ...
    private func generateOneofDescriptor(_ oneof: OneofNode) throws -> Google_Protobuf_OneofDescriptorProto {
    ...
    private func mapScalarType(_ type: TypeNode.ScalarType) throws -> Google_Protobuf_FieldDescriptorProto.TypeEnum {
        switch type {
	...
	private func processFileOptions(
		...
		for option in options {
			...
			case "java_generate_equals_and_hash":
				guard case .identifier(let value) = option.value else {
					throw DescriptorGeneratorError.invalidOptionValue("java_generate_equals_and_hash must be a boolean")
				}
			...
			case "java_generic_services":
				guard case .identifier(let value) = option.value else {
					throw DescriptorGeneratorError.invalidOptionValue("java_generic_services must be a boolean")
				}
	...
	private func processMessageOptions(
		...
		for option in options {
			...
			case "message_set_wire_format":
				guard case .identifier(let value) = option.value else {
					throw DescriptorGeneratorError.invalidOptionValue("message_set_wire_format must be a boolean")
				}
				messageOptions.messageSetWireFormat = value == "true"
				
			case "no_standard_descriptor_accessor":
				guard case .identifier(let value) = option.value else {
					throw DescriptorGeneratorError.invalidOptionValue("no_standard_descriptor_accessor must be a boolean")
				}
	...
	private func processMethodOptions(
		...
		for option in options {
			...
			case "idempotency_level":
				guard case .identifier(let value) = option.value else {
					throw DescriptorGeneratorError.invalidOptionValue("idempotency_level must be an identifier")
				}
				switch value.uppercased() {
				...
				default:
					throw DescriptorGeneratorError.invalidOptionValue("Invalid idempotency_level value: \(value)")
				}
