    ...
    private func validateFieldOptions() throws {
        for option in options {
            ...
            case "packed":
                ...
                if case .scalar = type {
                    guard isRepeated else {
                        throw ParserError.invalidFieldOption(option.name, "can only be specified for repeated scalar fields")
                    }
                } else {
                    throw ParserError.invalidFieldOption(option.name, "can only be specified for scalar types")
                }
...
private func isValidFieldName(_ name: String) -> Bool {
    ...
    return (first.isLowercase || first == "_") && name.allSatisfy { $0.isLetter || $0.isNumber || $0 == "_" }
}
