...
public class DescriptorGenerator {
    ...
    public func generate() -> FileDescriptor {
        ...
        // Create and return the FileDescriptor
    ...
    private func generateOption(_ option: OptionStatement) -> Option {
    ...
    private func mapOptionValue(_ value: OptionValue) -> OptionValue {
        switch value {
        case .string(let str):
            return .string(str)
        case .number(let num):
        ...
        case .boolean(let bool):
            return .boolean(bool)
        case .aggregate(let options):
            let mappedOptions = options.map { generateOption($0) }
            return .aggregate(mappedOptions)
        }
    }
}
